...
def generate_inquiry_pdf(person_name, start_date, end_date):
    try:
        df = pd.read_csv(CSV_FILE, dtype={
                         'reference_number': str}, keep_default_na=False)
        df['date'] = pd.to_datetime(df['date'], errors='coerce')
        ...
        filtered_df = df[
            ...
            # Only show payments made by me to this person
            (df['type'] == 'i_paid') &
            (df['date'] >= start_date) &
            (df['date'] <= end_date)
        ].copy()
        ...
        if filtered_df.empty:
            st.warning(
                f"No payments made to {person_name} found in the selected date range.")
            return None

        pdf = FPDF()
        pdf.add_page()

        PDFHelpers.generate_header(
            pdf, "INQUIRY REPORT - Details of Payments Made", person_name, start_date, end_date)

        pdf.ln(10)
        pdf.set_font('Helvetica', 'B', 9)
        headers = ['Date', 'Debit', 'Credit',
                   'Description', 'Serial ID', 'Receipt ID']
        col_widths = [25, 25, 25, 50, 25, 40]
        ...
        filename = f"inquiry_report_{person_name.replace(' ', '_')}_{start_str}_{end_str}.pdf"
        ...
        st.error(f"Error generating inquiry report: {e}")
        return None
...
def generate_bill_pdf(person_name, start_date, end_date, category=None):
    try:
        ...
        if filtered_df.empty:
            st.warning(
                f"No client expenses found for {person_name} in the selected date range.")
            return None
        ...
        # Header
        PDFHelpers.generate_header(
            pdf, "BILL REPORT - Client Expenses Details", person_name, start_date, end_date)
        ...
        for _, row in filtered_df.iterrows():
            ...
            # Serial ID
            pdf.cell(col_widths[4], 8, f"BILL-{serial_counter:03d}", 1, 0, 'C')
            pdf.cell(col_widths[5], 8, str(row.get(
                # Receipt ID
        ...
        start_str = start_date.strftime('%Y-%m-%d') if hasattr(start_date, 'strftime') else str(start_date)
        end_str = end_date.strftime('%Y-%m-%d') if hasattr(end_date, 'strftime') else str(end_date)
        filename = f"bill_report_{person_name.replace(' ', '_')}_{start_str}_{end_str}.pdf"
        filepath = os.path.join(os.getcwd(), filename)
        pdf.output(filepath)
        ...
        st.error(f"Error generating bill report: {e}")
        return None
...
def generate_invoice_pdf(person_name, start_date, end_date, category=None):
    try:
        ...
        payments_df['date'] = pd.to_datetime(
            payments_df['date'], errors='coerce')
        payments_df['amount'] = pd.to_numeric(
            payments_df['amount'], errors='coerce').fillna(0.0)

        expenses_df = pd.read_csv(CLIENT_EXPENSES_FILE, dtype={
                                  'original_transaction_ref_num': str, 'expense_person': str}, keep_default_na=False)
        ...
        if payments_filtered.empty and expenses_filtered.empty:
            st.warning(
                f"No transactions or expenses found for {person_name} in the selected date range to generate an invoice.")
            return None
        ...
        # Header
        PDFHelpers.generate_header(
            pdf, "INVOICE REPORT - Combined Transactions", person_name, start_date, end_date)
        ...
        for transaction in all_transactions:
            pdf.cell(col_widths[0], 8, transaction['date'].strftime('%Y-%m-%d'), border=1, align='C', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.cell(col_widths[1], 8, f"{transaction['debit']:,.2f}" if transaction['debit'] > 0 else "0.00", border=1, align='R', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.cell(col_widths[2], 8, f"{transaction['credit']:,.2f}" if transaction['credit'] > 0 else "0.00", border=1, align='R', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.cell(col_widths[3], 8, str(transaction['description'])[:20], border=1, align='L', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.cell(col_widths[4], 8, f"INV-{serial_counter:03d}", border=1, align='C', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.cell(col_widths[5], 8, str(transaction['reference'])[:15], border=1, align='C', new_x=XPos.RIGHT, new_y=YPos.TOP)
            pdf.ln()
        ...
        start_str = start_date.strftime('%Y-%m-%d') if hasattr(start_date, 'strftime') else str(start_date)
        end_str = end_date.strftime('%Y-%m-%d') if hasattr(end_date, 'strftime') else str(end_date)
        filename = f"invoice_report_{person_name.replace(' ', '_')}_{start_str}_{end_str}.pdf"
        filepath = os.path.join(os.getcwd(), filename)
        pdf.output(filepath)
        ...
        st.error(f"File system error generating invoice: {e}")
        return None
    except pd.errors.EmptyDataError as e:
        st.error(f"No data available for invoice: {e}")
        return None
    except ValueError as e:
        st.error(f"Invalid values in data for invoice: {e}")
        return None

# This function has been replaced by generate_invoice_pdf
